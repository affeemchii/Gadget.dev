    ...
    function AutoTableColumnHeader(props) {
        ...
        return (React.createElement(React.Fragment, null,
            React.createElement(TableHead, { key: column.identifier, className: `${stickyClass} bg-${isRowHovered ? "muted" : "background"}` },
                React.createElement("div", { className: `flex flex-row items-center gap-2 z-10 ` }, column.sortable ? (React.createElement(Button, { variant: "ghost", size: "sm", className: "cursor-pointer", onClick: () => {
                        const field = column.type === "CustomRenderer" ? column.sortByField : column.field;
                        if (field)
                            sort.handleColumnSort(field);
                    }, ...hoverProps }, ColumnHeaderLabel)) : (ColumnHeaderLabel)))));
    }
