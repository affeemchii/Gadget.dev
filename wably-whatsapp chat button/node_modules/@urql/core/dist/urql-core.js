...
var mapExchange = ({
  ...
  return ({
    ...
    return wonka.mergeMap(result => {
      ...
      if (operation.kind === 'teardown') return wonka.fromValue(operation);
      var newOperation = onOperation && onOperation(operation) || operation;
      return 'then' in newOperation ? wonka.takeUntil(wonka.filter(op => op.kind === 'teardown' && op.key === operation.key)(ops$))(wonka.fromPromise(newOperation)) : wonka.fromValue(newOperation);
    })(ops$)));
  };
};
...
var Client = function Client(opts) {
  ...
  var baseOpts = {
    ...
    fetch: opts.fetch,
    preferGetMethod: opts.preferGetMethod != null ? opts.preferGetMethod : 'within-url-limit',
