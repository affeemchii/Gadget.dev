...
export default function Index() {
  ...
  const [editMode, setEditMode] = useState<"status" | "description" | null>(null);
  ...
  // ── open / close helpers ──────────────────────────────────────────────────
  const openEdit = (record: any, mode: "status" | "description") => {
    setEditingProduct({
      id:     record.id,
    ...
    <s-page heading="Products">
              ...
              render: ({ record }: { record: any }) => (
                <div style={{ display: "flex", gap: "8px" }}>
                  <s-button
                    ...
                    onClick={(e: any) => { e.stopPropagation(); openEdit(record, "status"); }}
                  >
                    Edit Status
                  </s-button>
                  <s-button
                    commandFor="edit-product-modal"
                    onClick={(e: any) => { e.stopPropagation(); openEdit(record, "description"); }}
                  >
                    Edit Description
                  </s-button>
                </div>
      ...
      <s-modal
        ...
        heading={
          editingProduct
            ? editMode === "status"
              ? `Edit Status — ${editingProduct.title}`
      ...
      >
        ...
        {/* ── Status view ─────────────────────────────────────────────── */}
        {editingProduct && editMode === "status" && (
          <s-box padding="large">
            <s-stack gap="large">
              <s-text tone="neutral">
                Update the display status of this product in your store catalog.
              </s-text>
              <s-select
                label="Status"
                value={status}
        ...
        {/* ── Description (rich-text) view ─────────────────────────────── */}
        {editingProduct && editMode === "description" && (
          <s-box padding="large">
            <s-stack gap="base">
              <s-text tone="neutral">
              ...
              <div className="formatting-toolbar">
                ...
                <div className="toolbar-group">
                  <select
                    ...
                    defaultValue="14"
                    onChange={(e) => {
                      // execCommand fontSize uses 1-7 scale; use inline CSS instead
                      exec("fontSize", "7");
                      // override with real px via font-size on the <font> el
